#include <stddef.h>
#define PY_SSIZE_T_CLEAN
#include <Python.h>

#include "cgemm_kernel.h"

#include <string.h>
#include <stdlib.h>
#include <immintrin.h>

#include "cgemm_args.h"

// 64 alignement to fit full cache line
#define ALIGNEMENT 64

// Prefetech distance
#define prefetch_D 64
/*
C: U*V
A: W*V
B: U*W

Constraints:
- U * V <= 12 * (4 * 4)
- W % U = 0 ?
- W % V = 0 ?
- W * U + W * V <= 3200 (4000 hard)
- U % 8 = 0
- V % 8 = 0
- W = 2^n

*/
#define U 8
#define V 4
#define W 128 // Kernel constant
#define X 128 // Blocking constant


PyObject* aligned_memory_prepare(PyObject* self, PyObject* args) {
    size_t K;
    if (!PyArg_ParseTuple(args, "n", &K)) return NULL;

    PyGEMMArgs *gemm_args;
    gemm_args = (PyGEMMArgs *) PyGEMMArgsType.tp_alloc(&PyGEMMArgsType, 0);

    gemm_args->A = (double *) aligned_alloc(ALIGNEMENT, sizeof(double) * K * K);
    gemm_args->B = (double *) aligned_alloc(ALIGNEMENT, sizeof(double) * K * K);
    gemm_args->C = (double *) aligned_alloc(ALIGNEMENT, sizeof(double) * K * K);
    gemm_args->K = K;

    init_mat(gemm_args->A, K);
    init_mat(gemm_args->B, K);
    memset(gemm_args->C, 0, sizeof(double) * K * K);

    return (PyObject *)gemm_args;
}


typedef struct {
    __m256d c00, c10;
    __m256d c01, c11;
    __m256d c02, c12;
    __m256d c03, c13;
} ckernel;



// Continuous Sub B
static double* prepare_csb(dim size) {
    double* csb = (double *) aligned_alloc(ALIGNEMENT, sizeof(double) * size * U);
    return csb;
}

static void free_csb(double* csb){
    free(csb);
}


static void sparse_copy_B(f64ro sB, f64rw csb, dim K) {
    dim Ud = U * sizeof(double);
    for (int w = K - 1; w >= 0; w -= 4) {
        __builtin_prefetch(sB + (w - 0 - prefetch_D) * K, 0, 3);
        __builtin_prefetch(sB + (w - 1 - prefetch_D) * K, 0, 3);
        __builtin_prefetch(sB + (w - 2 - prefetch_D) * K, 0, 3);
        __builtin_prefetch(sB + (w - 3 - prefetch_D) * K, 0, 3);
        memcpy(csb + (w - 0) * U, sB + (w - 0) * K, Ud);
        memcpy(csb + (w - 1) * U, sB + (w - 1) * K, Ud);
        memcpy(csb + (w - 2) * U, sB + (w - 2) * K, Ud);
        memcpy(csb + (w - 3) * U, sB + (w - 3) * K, Ud);
    }
}

static void sparse_copy_B_ld(f64ro sB, f64rw csb, dim ldb, dim K) {
    dim Ud = U * sizeof(double);
    for (int w = K - 1; w >= 0; w -= 4) {
        __builtin_prefetch(sB + (w - 0 - prefetch_D) * ldb, 0, 3);
        __builtin_prefetch(sB + (w - 1 - prefetch_D) * ldb, 0, 3);
        __builtin_prefetch(sB + (w - 2 - prefetch_D) * ldb, 0, 3);
        __builtin_prefetch(sB + (w - 3 - prefetch_D) * ldb, 0, 3);
        memcpy(csb + (w - 0) * U, sB + (w - 0) * ldb, Ud);
        memcpy(csb + (w - 1) * U, sB + (w - 1) * ldb, Ud);
        memcpy(csb + (w - 2) * U, sB + (w - 2) * ldb, Ud);
        memcpy(csb + (w - 3) * U, sB + (w - 3) * ldb, Ud);
    }
}



static ckernel load_kernel_block(f64rw kC, dim K){
    ckernel core;

    core.c00 = _mm256_load_pd(kC);
    core.c10 = _mm256_load_pd(kC + 4);

    core.c01 = _mm256_load_pd(kC + K);
    core.c11 = _mm256_load_pd(kC + 4 + K);

    core.c02 = _mm256_load_pd(kC + 2 * K);
    core.c12 = _mm256_load_pd(kC + 4 + 2 * K);

    core.c03 = _mm256_load_pd(kC + 3 * K);
    core.c13 = _mm256_load_pd(kC + 4 + 3 * K);

    return core;
}

static ckernel load_kernel_block_ld(f64rw kC, dim ldc){
    ckernel core;

    core.c00 = _mm256_load_pd(kC + 0 + 0 * ldc);
    core.c10 = _mm256_load_pd(kC + 4 + 0 * ldc);

    core.c01 = _mm256_load_pd(kC + 0 + 1 * ldc);
    core.c11 = _mm256_load_pd(kC + 4 + 1 * ldc);

    core.c02 = _mm256_load_pd(kC + 0 + 2 * ldc);
    core.c12 = _mm256_load_pd(kC + 4 + 2 * ldc);

    core.c03 = _mm256_load_pd(kC + 0 + 3 * ldc);
    core.c13 = _mm256_load_pd(kC + 4 + 3 * ldc);

    return core;
}

static ckernel prepare_kernel_block(){
    ckernel core;

    core.c00 = _mm256_setzero_pd();
    core.c10 = _mm256_setzero_pd();

    core.c01 = _mm256_setzero_pd();
    core.c11 = _mm256_setzero_pd();

    core.c02 = _mm256_setzero_pd();
    core.c12 = _mm256_setzero_pd();

    core.c03 = _mm256_setzero_pd();
    core.c13 = _mm256_setzero_pd();

    return core;
}

static void store_kernel_block(f64rw kC, ckernel* core,  dim K){
    _mm256_store_pd(kC, core->c00);
    _mm256_store_pd(kC + 4, core->c10);

    _mm256_store_pd(kC + K, core->c01);
    _mm256_store_pd(kC + 4 + K, core->c11);

    _mm256_store_pd(kC + 2 * K, core->c02);
    _mm256_store_pd(kC + 4 + 2 * K, core->c12);

    _mm256_store_pd(kC + 3 * K, core->c03);
    _mm256_store_pd(kC + 4 + 3 * K, core->c13);

}

static void store_kernel_block_ld(f64rw kC, ckernel* core, dim ldc){
    _mm256_store_pd(kC + 0 + 0 * ldc, core->c00);
    _mm256_store_pd(kC + 4 + 0 * ldc, core->c10);

    _mm256_store_pd(kC + 0 + 1 * ldc, core->c01);
    _mm256_store_pd(kC + 4 + 1 * ldc, core->c11);

    _mm256_store_pd(kC + 0 + 2 * ldc, core->c02);
    _mm256_store_pd(kC + 4 + 2 * ldc, core->c12);

    _mm256_store_pd(kC + 0 + 3 * ldc, core->c03);
    _mm256_store_pd(kC + 4 + 3 * ldc, core->c13);

}

static void ckernel8x4_csb(f64ro srA, f64ro csb, ckernel* core, dim K){

    for (size_t w = 0; w < W; ++w) {

        __m256d b00 = _mm256_load_pd(csb + w * U);
        __m256d b10 = _mm256_load_pd(csb + 4 + w * U);

        __m256d a000 =_mm256_broadcast_sd(srA + w);
        core->c00 = _mm256_fmadd_pd(a000, b00, core->c00);
        core->c10 = _mm256_fmadd_pd(a000, b10, core->c10);

        __m256d a010 =_mm256_broadcast_sd(srA + K + w);
        core->c01 = _mm256_fmadd_pd(a010, b00, core->c01);
        core->c11 = _mm256_fmadd_pd(a010, b10, core->c11);

        __m256d a020 =_mm256_broadcast_sd(srA + 2 * K + w);
        core->c02 = _mm256_fmadd_pd(a020, b00, core->c02);
        core->c12 = _mm256_fmadd_pd(a020, b10, core->c12);

        __m256d a030 =_mm256_broadcast_sd(srA + 3 * K + w);
        core->c03 = _mm256_fmadd_pd(a030, b00, core->c03);
        core->c13 = _mm256_fmadd_pd(a030, b10, core->c13);
    }
}

static void ckernel8x4_csb_ld(f64ro srA, dim lda, f64ro csb, ckernel* core, dim K){

    for (size_t w = 0; w < W; ++w) {

        __m256d b00 = _mm256_load_pd(csb + 0 + w * U);
        __m256d b10 = _mm256_load_pd(csb + 4 + w * U);

        __m256d a000 =_mm256_broadcast_sd(srA + 0 * lda + w);
        core->c00 = _mm256_fmadd_pd(a000, b00, core->c00);
        core->c10 = _mm256_fmadd_pd(a000, b10, core->c10);

        __m256d a010 =_mm256_broadcast_sd(srA + 1 * lda + w);
        core->c01 = _mm256_fmadd_pd(a010, b00, core->c01);
        core->c11 = _mm256_fmadd_pd(a010, b10, core->c11);

        __m256d a020 =_mm256_broadcast_sd(srA + 2 * lda + w);
        core->c02 = _mm256_fmadd_pd(a020, b00, core->c02);
        core->c12 = _mm256_fmadd_pd(a020, b10, core->c12);

        __m256d a030 =_mm256_broadcast_sd(srA + 3 * lda + w);
        core->c03 = _mm256_fmadd_pd(a030, b00, core->c03);
        core->c13 = _mm256_fmadd_pd(a030, b10, core->c13);
    }
}


static void ckernel8x4_csb_unrolled(f64ro srA, f64ro csb, ckernel* core, dim K){

    for (size_t w = 0; w < W; w+=2) {

        __m256d b00 = _mm256_load_pd(csb + w * U);
        __m256d b10 = _mm256_load_pd(csb + 4 + w * U);

        __m256d a000 =_mm256_broadcast_sd(srA + w);
        core->c00 = _mm256_fmadd_pd(a000, b00, core->c00);
        core->c10 = _mm256_fmadd_pd(a000, b10, core->c10);

        __m256d a010 =_mm256_broadcast_sd(srA + K + w);
        core->c01 = _mm256_fmadd_pd(a010, b00, core->c01);
        core->c11 = _mm256_fmadd_pd(a010, b10, core->c11);

        __m256d a020 =_mm256_broadcast_sd(srA + 2 * K + w);
        core->c02 = _mm256_fmadd_pd(a020, b00, core->c02);
        core->c12 = _mm256_fmadd_pd(a020, b10, core->c12);

        __m256d a030 =_mm256_broadcast_sd(srA + 3 * K + w);
        core->c03 = _mm256_fmadd_pd(a030, b00, core->c03);
        core->c13 = _mm256_fmadd_pd(a030, b10, core->c13);

        __m256d b20 = _mm256_load_pd(csb + (w + 1) * U);
        __m256d b30 = _mm256_load_pd(csb + 4 + (w + 1) * U);

        __m256d a040 =_mm256_broadcast_sd(srA + (w + 1));
        core->c00 = _mm256_fmadd_pd(a040, b20, core->c00);
        core->c10 = _mm256_fmadd_pd(a040, b30, core->c10);

        __m256d a050 =_mm256_broadcast_sd(srA + K + (w + 1));
        core->c01 = _mm256_fmadd_pd(a050, b20, core->c01);
        core->c11 = _mm256_fmadd_pd(a050, b30, core->c11);

        __m256d a060 =_mm256_broadcast_sd(srA + 2 * K + (w + 1));
        core->c02 = _mm256_fmadd_pd(a060, b20, core->c02);
        core->c12 = _mm256_fmadd_pd(a060, b30, core->c12);

        __m256d a070 =_mm256_broadcast_sd(srA + 3 * K + (w + 1));
        core->c03 = _mm256_fmadd_pd(a070, b20, core->c03);
        core->c13 = _mm256_fmadd_pd(a070, b30, core->c13);
    }
}

// full compute of a C block of shape (U, V) that depends on row of A (K, V) and column of B (U, K)
static void c_block(f64ro rowA, f64ro ccb, f64rw kC, dim K) {
    dim KW = K/W;

    // Load C into registers
    ckernel core = prepare_kernel_block();

    for (size_t kw = 0; kw < KW; ++kw) {
        // sub row of A: (W, V)
        f64ro srA = rowA + kw * W;
        // sub col of B: (X, U)
        f64ro csb = ccb + kw * W;

        // ckernel8x4(srA, scB, kC, K);
        ckernel8x4_csb(srA, csb, &core, K);
        // ckernel8x4_csb_unrolled(srA, csb, &core, K);
    }

    store_kernel_block(kC, &core, K);

}


static void partial_c_block(f64ro srA, f64ro csb, f64rw kC, dim K) {

    // Load C into registers
    ckernel core = load_kernel_block(kC, K);

    ckernel8x4_csb(srA, csb, &core, K);

    store_kernel_block(kC, &core, K);

}

static void c_block_ld(f64ro rowA, dim lda, f64ro ccb, f64rw kC, dim ldc, dim K) {
    dim KW = K/W;

    // Load C into registers
    // ckernel core = load_kernel_block_ld(kC, ldc);
    ckernel core = prepare_kernel_block();

    for (size_t kw = 0; kw < KW; ++kw) {
        // sub row of A: (W, V)
        f64ro srA = rowA + kw * W;
        // sub col of B: (X, U)
        f64ro csb = ccb + kw * W;

        // ckernel8x4(srA, scB, kC, K);
        ckernel8x4_csb_ld(srA, lda, csb, &core, K);
        // ckernel8x4_csb_unrolled(srA, csb, &core, K);
    }

    store_kernel_block_ld(kC, &core, ldc);

}


// A @ B = C
//       j ->
//     i B B B
//   K | B B B
//    \/ B B B
// A A A C C C
// A A A C C C
// A A A C C C
void kernel_compute_intern(f64ro A, f64ro B, f64rw C, dim K) {
    dim KU = K/U;
    dim KV = K/V;

    // Continuous Column B
    f64rw ccb = prepare_csb(K);

    for (size_t ku = 0; ku < KU; ++ku) {

        // todo: extract B sublock so that it becomes continuous
        // Column of B: (U, K)
        f64ro colB = B + ku * U;
        sparse_copy_B(colB, ccb, K);
        // sparse_copy_B_2(colB, ccb, K);
        for (size_t kv = 0; kv < KV; ++kv) {
            // kC: kernel size block of C: (U,V)
            f64rw kC = C + ku * U + kv * V * K;

            // Row of A: (K, V)
            f64ro rowA = A + kv * V * K;
            c_block(rowA, ccb, kC, K);
        }
    }

    free_csb(ccb);
}


void kernel_compute_intern_blocked(f64ro A, f64ro B, f64rw C, dim K) {
    dim KU = K/U;
    dim KV = K/V;
    dim KX = K/X;
    f64rw csb = prepare_csb(X);

    for (size_t kx = 0; kx < KX; ++kx) {
        for (size_t ku = 0; ku < KU; ++ku) {
            // Column of B: (U, K)
            f64ro colB = B + ku * U;

            sparse_copy_B(colB + kx * X, csb, X);

            for (size_t kv = 0; kv < KV; ++kv) {


                // f64ro csb = ccb + kx * X;

                // kC: kernel size block of C: (U,V)
                f64rw kC = C + ku * U + kv * V * K;

                // Row of A: (K, V)
                f64ro rowA = A + kv * V * K;
                f64ro srA = rowA + kx * X;
                // Prefetch next block of A
                // __builtin_prefetch(A + (kv + 1) * V * K + (K + 0), 0, 3);
                // __builtin_prefetch(A + (kv + 1) * V * K + (K + 1), 0, 3);
                // __builtin_prefetch(A + (kv + 1) * V * K + (K + 2), 0, 3);
                // __builtin_prefetch(A + (kv + 1) * V * K + (K + 3), 0, 3);


                partial_c_block(srA, csb, kC, K);
            }
        }
    }
    free_csb(csb);
}

void kernel_compute_intern_ld(f64ro A, dim lda, f64ro B, dim ldb, f64rw C, dim ldc, dim K) {
    dim KU = K/U;
    dim KV = K/V;

    // Continuous Column B
    f64rw ccb = prepare_csb(K);

    for (size_t ku = 0; ku < KU; ++ku) {

        // todo: extract B sublock so that it becomes continuous
        // Column of B: (U, K)
        f64ro colB = B + ku * U;
        sparse_copy_B_ld(colB, ccb, ldb, K);
        // sparse_copy_B_2(colB, ccb, K);
        for (size_t kv = 0; kv < KV; ++kv) {
            // kC: kernel size block of C: (U,V)
            f64rw kC = C + ku * U + kv * V * ldc;

            // Row of A: (K, V)
            f64ro rowA = A + kv * V * lda;
            c_block_ld(rowA, lda, ccb, kC, ldc, K);
        }
    }

    free_csb(ccb);
}

void kernel_compute_intern_master(f64ro A, f64ro B, f64rw C, dim K) {
    dim D = 1024;
    kernel_compute_intern_ld(A, K, B, K, C, K, D);
    // kernel_compute_intern_ld(A + D, K, B + D, K, C + D, K, D);
    // kernel_compute_intern_ld(A + D * K, K, B + D * K, K, C + D * K, K, D);
    // kernel_compute_intern_ld(A + D + D * K, K, B + D + D * K, K, C + D + D * K, K, D);
}


PyObject* kernel_compute(PyObject* self, PyObject* args) {

    PyGEMMArgs* gemm_args;

    if (!PyArg_ParseTuple(args, "O!", &PyGEMMArgsType, &gemm_args)) return NULL;

    double * A = gemm_args->A;
    double * B = gemm_args->B;
    double * C = gemm_args->C;
    const size_t K = gemm_args->K;


    // kernel_compute_intern(A, B, C, K);
    // kernel_compute_intern_blocked(A, B, C, K);
    // kernel_compute_intern_ld(A, K, B, K, C, K, K);
    kernel_compute_intern_master(A, B, C, K);

    Py_INCREF(Py_None);
    return Py_None;
}

// NAUGHTY CODE CORNER

// Continuous Sub A
static double* prepare_csa(dim size) {
    double* csa = (double *) aligned_alloc(ALIGNEMENT, sizeof(double) * size * V);
    return csa;
}

static void free_csa(double* csa){
    free(csa);
}


static void pack_csa(f64ro sA, f64rw csa, dim K) {
    for (int k = 0; k < K; ++k) {
        f64rw dest = csa + k * 4;
        f64ro src = sA + k;

        dest[0] = src[0 * K];
        dest[1] = src[1 * K];
        dest[2] = src[2 * K];
        dest[3] = src[3 * K];
    }
}
static void ckernel8x4_csb_csa(f64ro csa, f64ro csb, ckernel* core, dim K){

    for (size_t w = 0; w < W; ++w) {

        __m256d b00 = _mm256_load_pd(csb + w * U);
        __m256d b10 = _mm256_load_pd(csb + 4 + w * U);

        __m256d a000 =_mm256_broadcast_sd(csa + w);
        core->c00 = _mm256_fmadd_pd(a000, b00, core->c00);
        core->c10 = _mm256_fmadd_pd(a000, b10, core->c10);

        __m256d a010 =_mm256_broadcast_sd(csa + w + 1);
        core->c01 = _mm256_fmadd_pd(a010, b00, core->c01);
        core->c11 = _mm256_fmadd_pd(a010, b10, core->c11);

        __m256d a020 =_mm256_broadcast_sd(csa + w + 2);
        core->c02 = _mm256_fmadd_pd(a020, b00, core->c02);
        core->c12 = _mm256_fmadd_pd(a020, b10, core->c12);

        __m256d a030 =_mm256_broadcast_sd(csa + w + 3);
        core->c03 = _mm256_fmadd_pd(a030, b00, core->c03);
        core->c13 = _mm256_fmadd_pd(a030, b10, core->c13);
    }
}
// C block with all source packed
static void c_block_ap(f64ro cra, f64ro ccb, f64rw kC, dim K) {
    dim KW = K/W;

    // Load C into registers
    ckernel core = prepare_kernel_block();

    for (size_t kw = 0; kw < KW; ++kw) {
        // sub row of A: (W, V)
        f64ro srA = cra + kw * W;
        // sub col of B: (X, U)
        f64ro csb = ccb + kw * W;

        ckernel8x4_csb_csa(srA, csb, &core, K);
    }

    store_kernel_block(kC, &core, K);

}
void kernel_compute_intern_ABpacked(f64ro A, f64ro B, f64rw C, dim K) {
    dim KU = K/U;
    dim KV = K/V;

    // Continuous Column B
    f64rw ccb = prepare_csb(K);
    // Continuous Row A (Continuous for kernel perspective)
    f64rw cra = prepare_csa(K);

    for (size_t ku = 0; ku < KU; ++ku) {

        // todo: extract B sublock so that it becomes continuous
        // Column of B: (U, K)
        f64ro colB = B + ku * U;
        sparse_copy_B(colB, ccb, K);
        for (size_t kv = 0; kv < KV; ++kv) {
            // kC: kernel size block of C: (U,V)
            f64rw kC = C + ku * U + kv * V * K;
            // Row of A: (K, V)
            f64ro rowA = A + kv * V * K;
            pack_csa(rowA, cra, K);
            c_block_ap(cra, ccb, kC, K);
        }
    }

    free_csb(ccb);
    free_csa(cra);
}
